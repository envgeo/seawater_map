...
import envgeo_utils  # さっき作った utils を呼ぶ
...
# ログ管理用
print(f"--------05_3D4D_visualizer_uploader_{version}--------")
...
st.header(f'3D visualizer uploader ({version})')

# 純粋なテキスト
# st.text('2024/12/14 test version')
# st.text('アップしたファイルはメモリ上に一時的に格納されるだけです')
st.text('The uploaded files are only temporarily cached in memory.')
...
if uploaded_file is not None:
    ...
    coastline_plot = st.radio("draw a coastline ", ("NO","JAPAN", "WORLD"), horizontal=True, args=[1, 0])
    
    if coastline_plot == "JAPAN":

    
        #海岸線を重ね書きする場合
        # 海岸線の座標データを手動で用意
        coastline_excel = 'coastline/japan_coast_line.xlsx'
        # coastline_excel = 'world_coastline_coordinates_110m.xlsx'  
    ...
    coastline_plo_3D = st.radio("draw a coastline on 3D plot", ("JAPAN", "WORLD", "NO"), horizontal=True, args=[1, 0])
...
#データフレームを作ってexcelで書き出し，ダウンロード
    ...
    try:
        ...
        st.download_button(
            label="Excel海洋d18Oデータサンプル(sample dataset: from Kodama et al., 2024.)",
            data=excel_data,
