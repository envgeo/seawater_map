...
##############################################################################
    ...
    # # 【重要】描画専用のデータフレームを作成し、インデックスをリセットしてズレを防止します
    # df_plot_ts = df1.reset_index(drop=True)

    # fig_fixed_TS = px.scatter(
    #     df_plot_ts, # リセット済みのデータを使用
    #     x="Salinity", y="Temperature_degC", 
    #     color=target_item, color_continuous_scale=c_scale_final,
    #     # hover_dataを辞書形式にして、どの列を表示するか確実に指定します
    #     hover_data={
    #         "Salinity": True,
    #         "Temperature_degC": True,
    #         "lat": True,
    ...
    # 【重要】描画専用のデータフレームを作成し、インデックスをリセットしてズレを防止します
    df_plot_d18o = df1.reset_index(drop=True)

    fig_d18O = px.scatter(
        df_plot_d18o, # リセット済みのデータを使用
        x="Salinity", y="d18O", 
        color=target_item, color_continuous_scale=c_scale_final,
        # hover_dataを辞書形式にすることで、より厳密に紐付けます
        hover_data={
            "Salinity": True,
            "d18O": True,
            "lat": True,
