...
print(f"--------31_salinity-d18O_reg-line_plot_({version})--------")
...
def main():
    ...
    # タイトル
    st.header(f'Salinity-δ18O ({version})')
    ...
    # データソース選択 old
    # ref_data = st.radio("data source (see home>about):", ("Kodama et al. (2024) only", "Kodama et al. (2024) with other reports", "with NASA_database"), horizontal=True, args=[1, 0])
...
##############################################################################
        ...
        # 例：列名が "RockType" の場合
        Transect_list = df_original["Transect"].dropna().unique().tolist()
        ...
        d18O_min, d18O_max = st.slider(label=r'$\delta^{18}$O scale selected',
                                    min_value=-30.0,
                                    max_value=5.0,
                                    value=(-5.0, 1.0),
                                    )
...
##################################選択データ表示　2024/10/07###################################################################################################################
    ...
    # print(f"--------11_salinity-d18O_reg-line_plot_({version})--------")
