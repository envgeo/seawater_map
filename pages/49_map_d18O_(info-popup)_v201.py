...
print(("--------49_surface_map_d18O_(info-popup)_v201.py-------"))
...
st.header('sea surface d18O mapping Ver.201')
st.subheader('(info-popup)')
...
# Transect013 = 'sECS'					  
# Transect014 = 'Nansei'
# # Transect015 = 'ここの新規項目を入力'
# # Transect016 = 'ここの新規項目を入力'
# # Transect017 = 'ここの新規項目を入力'
# # Transect018 = 'ここの新規項目を入力'
# # Transect019 = 'ここの新規項目を入力'
...
######  scalebarで制御してsubmitする場合 ################
    ...
    # m = folium.Map(location=[35, 135], tiles="OpenStreetMap", zoom_start=5)
    # m = folium.Map(location=[35, 135], tiles="OpenStreetMap", zoom_start=5)
    m = folium.Map(location=[35, 135], tiles="OpenStreetMap", zoom_start=5)
...
########### データ数を表示
   ...
   ########### データ数を表示　ここまで     
    ...
    for i, row in df1.iterrows():
        pop=f"Transect:{row['Transect']} <br> Lon: {row['Longitude_degE']}E <br> Lat: {row['Latitude_degN']}N <br> Depth: {row['Depth_m']}m <br> Date: {row['Date']} <br> Cruise: {row['Cruise']} <br> Station: {row['Station']} <br> Salinity: {row['Salinity']} <br> Temp: {row['Temperature_degC']} <br> d18O: {row['d18O']} <br> dD: {row['dD']} <br> ref.: {row['reference']}" 
        folium.Marker(
            # 緯度と経度を指定
