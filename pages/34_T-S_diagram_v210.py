...
def main():
    ...
    # タイトル
    st.header(f'Temperature-Salinity ({version})')
...
##############################################################################
        ...
        if X_Y_add2 == 1:
            ...
            for sheet_num_add in sheet_num_add:    
                ...
                if X_Y_C_add_each == 1:
                    ...
                    # st.write(df_empty)
                    data_found = len(df1["Temperature_degC"])
                    data_found_num = str(len(df1["Temperature_degC"]))
    ...
    ##################################選択データ表示　2024/10/07###################################################################################################################
                    ...
                    st.markdown("##### :red[--- The plot-range can be adjusted using the [fig scale] setting---]")
        ...
        # cnt = np.linspace(sigma_theta.min(), sigma_theta.max(),2222)
