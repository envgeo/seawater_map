...
#     st.pyplot(fig)
    ...
    ###############################################################################################
    #全体のタイトル名　　手入力
    main_title = r'SEAWATER $\delta^{18}$O MAP WEB (b02)'
    sub_title = 'Lon:'+str(sld_lon_min)+'-'+str(sld_lon_max)+', Lat:'+str(sld_lat_min)+'-'+str(sld_lat_max)+', Y:'+str(sld_year_min)+'-'+str(sld_year_max)+', M:'+str(sld_month_min)+'-'+str(sld_month_max)+', S:'+str(sld_sal_min)+'-'+str(sld_sal_max)+', D:'+str(sld_depth_min)+'-'+str(sld_depth_max)+'m'
    sub_title2 = ''
    ...
    ax_cmap = ax.scatter(df1["Longitude_degE"], df1["Latitude_degN"], 
                         ...
                         # vmin=-1.5, vmax=1.0, transform=ccrs.PlateCarree(), zorder=1)
    ...
    # st.download_button(label="Download Scatter Map", data=img_scatter, file_name=fn_scatter, mime="image/png")
    ...
    # =============================================================================
    # st.write("---")
    # st.write(":blue[--- Interpolated Contour Map ---]")
    ...
    # 1. 補完計算
    grid_lon = np.linspace(map_lon_min, map_lon_max, 250)
    grid_lat = np.linspace(map_lat_min, map_lat_max, 250)
    X, Y = np.meshgrid(grid_lon, grid_lat)
    Z = griddata((df1["Longitude_degE"], df1["Latitude_degN"]), df1['d18O'], (X, Y), method='linear')
    ...
    # タイトル設定
    title_contour = title_head2.replace('(Scatter)', '(Interpolated)')
    ...
    # st.download_button(label="Download Contour Map", data=img_contour, file_name=fn_contour, mime="image/png")

    # st.pyplot(fig_contour)
    
    # --- 以降、plotlyのマップとデータフレーム表示 ---
    ...
    map_type = st.radio(":blue[Map Type:]", ("Scatter Map", "Contour Map"), horizontal=True, args=[1, 0])
