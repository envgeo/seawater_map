...
def main():
    ...
    # reference選択
    selected_reference = st.sidebar.multiselect(
        'Select Reference:',
        df1['reference'].unique(),
        default=df1['reference'].unique()
    )
    df1 = df1[df1['reference'].isin(selected_reference)]
    ...
    # Year選択 (JAPAN版にあったロジック)
    # selected_year = st.sidebar.multiselect(
    #     'Year:',
    #     df1['Year'].unique(),
    #     default=df1['Year'].unique()
    # )
    # df1 = df1[df1['Year'].isin(selected_station)]
    
    
    # # Month選択 (JAPAN版にあったロジック)
    # selected_station = st.sidebar.multiselect(
    #     'Select Month:',
    #     df1['Month'].unique(),
    #     default=df1['Month'].unique()
    # )
    # df1 = df1[df1['Month'].isin(selected_station)]
    
    

    # 緯度フィルタ
    max_d = int(df1['Latitude_degN'].max())
    min_d = int(df1['Latitude_degN'].min())
    depth_limit = st.sidebar.slider('Latitude filter:', min_d, max_d, (min_d, max_d))
    df1 = df1[(df1['Latitude_degN'] >= depth_limit[0]) & (df1['Latitude_degN'] <= depth_limit[1])]
    
    # 経度フィルタ
    max_d = int(df1['Longitude_degE'].max())
    min_d = int(df1['Longitude_degE'].min())
    depth_limit = st.sidebar.slider('Longitude filter:', min_d, max_d, (min_d, max_d))
    df1 = df1[(df1['Longitude_degE'] >= depth_limit[0]) & (df1['Longitude_degE'] <= depth_limit[1])]
